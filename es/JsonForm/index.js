...
import FormLabel from './items/Label'; ////////////////////////////// ====v2==== ////////////////////////////
...
import FormPasswordV2 from './items/v2/Password';
import FormDatePickerV2 from './items/v2/DatePicker';
...
export var getFormItem = function getFormItem(_a, form, labelClassName, itemCol, itemRow, index, hide) {
  ...
  if (FormPasswordV2.typeList.includes(type)) {
    return getColChildren(React.createElement(FormPasswordV2, __assign({
      key: String(name),
  ...
  if (FormCascaderV2.typeList.includes(type)) {
    return getColChildren(React.createElement(FormCascaderV2, __assign({
      key: String(index),
      labelClassName: labelClassName
    }, field, {
      type: type,
  ...
  if (FormDatePickerV2.typeList.includes(type)) {
    return getColChildren(React.createElement(FormDatePickerV2, __assign({
      key: String(index),
      labelClassName: labelClassName
    }, field, {
      type: type,
...
var JsonForm = function JsonForm(props, ref) {
  ...
  var getValues = useCallback(function (targetFieldList) {
    ...
    target.map(function (field) {
      ...
      if (FormSwitch.typeList.includes(type)) {
      ...
      } else if (FormLabel.typeList.includes(type) || FormLabelV2.typeList.includes(type)) {// 没有值需要获取
      } else if (FormPassword.typeList.includes(type) || FormPasswordV2.typeList.includes(type)) {
        var _d = field,
        ...
        if (FormInput.typeList.includes(type) || FormTextArea.typeList.includes(type) || FormSelect.typeList.includes(type) || FormDatePicker.typeList.includes(type) || FormSelectV2.typeList.includes(type) || FormInputV2.typeList.includes(type)) {
          values[name_3] = getFormatterFunc(formatterName, formatter["null"])(form.getFieldValue(name_3));
          ...
          if (Array.isArray(name_3)) {
            values[name_3[0]] = getFormatterFunc(formatterName, formatter["null"])(form.getFieldValue(name_3[0]));
          } else {
            values[name_3] = getFormatterFunc(formatterName, formatter["null"])(form.getFieldValue(name_3));
          }
        }
